document.addEventListener("DOMContentLoaded", function () {
    ...
    if (eventsData && eventsData.length != 0) {
        for (var eventIndex in eventsData) {
            configurationJsTabs.addNewRowSection(
...
varienTabs.prototype.addNewRowSection = function (
    ...
    eventNameField.name = `event[${eventIndex}][eventName]`;
    eventNameField.className = "eventname";
    eventNameField.placeholder = "Event Name";
    if (eventData && eventData.rows && eventData.rows[0].event) {
        eventNameField.value = eventData.rows[0].event;
    }
    ...
    addButton.onclick = function () {
        var newRow = configurationJsTabs.createRow(eventIndex);
        if (newRow) {
    ...
    if (eventData && eventData.rows) {
        eventData.rows.forEach(function (rowData, rowIndex) {
            var row = configurationJsTabs.createRow(eventIndex, rowData, rowIndex);
            if (row) {
                section.appendChild(row);
            }
        });
    } else {
        var initialRow = configurationJsTabs.createRow(eventIndex);
        if (initialRow) {
...
varienTabs.prototype.createRow = function (eventIndex, rowData, rowIndex) {
    ...
    dropdown1.name = `event[${eventIndex}][rows][${rowIndex}][condition_dropdown]`;
    if (rowData && rowData.condition) {
        dropdown1.value = rowData.condition;
    }

    // Operator Dropdown
    var dropdown2 = this.createDropdown(["Like", "%Like%", "=="]);
    dropdown2.name = `event[${eventIndex}][rows][${rowIndex}][operator_dropdown]`;
